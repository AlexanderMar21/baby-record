<template>
	<div class="flex justify-center flex-col items-center max-w-440px mx-auto max-h-full">
		<h1 class="text-4xl mb-4">Milk Recorder</h1>
		...
		<form @submit.prevent="addMilk" class="w-full h-full">
			...
			<div class="flex items-center justify-between gap-4">
				...
				<button class="btn flex-1 inline-block mt-3" type="submit">Save</button>
			</div>
		</form>
		...
		<section class="w-full">
			...
			>
				<li v-for="record in milkRecords" :key="record.id" class="flex items-center justify-between mb-1">
					<span>
...
const addMilk = () => {
	useMilkStore().addMilkRecord({
		...
		amount: 10,
		time: new Date(`${date.value}T${time.value}`),
		id: create_UUID(),
	});
};
</script>
